...
class FixedBottomNavigationTab extends BottomNavigationTab {
    ...
    void init() {
        ...
        sharedPreferences = getContext().getSharedPreferences("ConvenienceNet", Context.MODE_PRIVATE);
        int screenWidth = sharedPreferences.getInt("screenWidth", 720);
        Log.i("ssssssssss",screenWidth+"");
        if (screenWidth < 720)
            labelScale = getResources().getDimension(R.dimen.fixed_label_inactive2) / getResources().getDimension(R.dimen.fixed_label_active2);
        else {
